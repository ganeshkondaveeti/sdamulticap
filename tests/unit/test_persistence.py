...
def test_persistence_rejects_plaintext_secret_payloads(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="secret material"):
        audit.append(job_id="job-1", event="credential", payload={"token": "not-on-disk"})
